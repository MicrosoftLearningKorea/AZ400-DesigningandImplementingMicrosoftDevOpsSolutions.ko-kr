...
# <a name="implementing-github-actions-for-cicd"></a>CI/CD에 대한 GitHub Actions 구현
...
### <a name="exercise-0-import-eshoponweb-to-your-github-repository"></a>연습 0: GitHub 리포지토리로 eShopOnWeb 가져오기
...
#### <a name="task-1-create-a-public-repository-in-github-and-import-eshoponweb"></a>작업 1: GitHub에서 퍼블릭 리포지토리 만들기 및 eShopOnWeb 가져오기
...
### <a name="exercise-1-setup-your-github-repository-and-azure-access"></a>연습 1: GitHub 리포지토리 및 Azure 액세스 설정
...
#### <a name="task-1-create-an-azure-service-principal-and-save-it-as-github-secret"></a>작업 1: Azure 서비스 주체 만들기 및 GitHub 비밀로 저장
...
1. 이 명령은 JSON 개체를 출력합니다. 나중에 이를 워크플로에 대한 GitHub 비밀로 보관하고 복사하게 됩니다. JSON에는 Azure AD 애플리케이션 ID(서비스 주체)의 이름으로 Azure에 대해 인증하는 데 사용되는 식별자가 포함되어 있습니다.
...
1. 리포지토리 페이지에서 **설정**으로 이동하여 **비밀 > 작업을** 클릭합니다. **새 리포지토리 비밀**을 클릭합니다.
...
#### <a name="task-2-modify-and-execute-the-github-workflow"></a>작업 2: GitHub 워크플로 수정 및 실행

이 작업에서는 지정된 GitHub 워크플로를 수정하고 실행하여 사용자의 고유한 구독에 솔루션을 배포합니다.

1. 브라우저 창에서 **eShopOnWeb** GitHub 리포지토리로 돌아갑니다.
1. 리포지토리 페이지에서 **코드**로 이동하고 **eShopOnWeb/.github/workflows/eshoponweb-cicd.yml** 파일을 엽니다. 이 워크플로는 지정된 .NET 6 웹 사이트 코드에 대한 CI/CD 프로세스를 정의합니다.
...
1. **env** 섹션에서 다음과 같이 변경합니다.
    ...
    - (선택 사항) [LOCATION](https://azure.microsoft.com/en-gb/explore/global-infrastructure/geographies/#geographies)에 가장 가까운 **Azure 지역**을 선택할 수 있습니다. 예를 들어 'eastus', 'eastasia', 'westus' 등이 있습니다.
...
#### <a name="task-3-review-github-workflow-execution"></a>작업 3: GitHub 워크플로 실행 검토
...
1. 워크플로가 완료될 때까지 기다립니다. **요약**에서는 두 가지 워크플로 작업, 즉 실행에서 유지된 상태 및 아티팩트를 볼 수 있습니다. 각 작업을 클릭하여 로그를 검토할 수 있습니다.
    ...
    ![워크플로 성공](images/gh-action-success.png)
...
#### <a name="optional-task-4-add-manual-approval-pre-deploy-using-github-environments"></a>(선택 사항) 작업 4: GitHub 환경을 사용하여 수동 승인 사전 배포 추가
...
1. **배포** 작업 섹션에서 **개발**이라는 **환경**의 참조를 찾을 수 있습니다. GitHub에서 사용한 [환경](https://docs.github.com/en/actions/deployment/targeting-different-environments/using-environments-for-deployment)은 대상에 대한 보호 규칙(및 비밀)을 추가합니다.
