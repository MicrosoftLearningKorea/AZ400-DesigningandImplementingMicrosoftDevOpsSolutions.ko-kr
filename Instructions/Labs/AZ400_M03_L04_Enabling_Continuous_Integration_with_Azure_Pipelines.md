...
먼저 [eshoponweb-ci-pr.yml](https://github.com/MicrosoftLearning/eShopOnWeb/blob/main/.ado/eshoponweb-ci-pr.yml)이라는 빌드 파이프라인을 가져옵니다.

1. **파이프라인 > 파이프라인**으로 이동합니다.
2. **파이프라인 만들기** 또는 **새 파이프라인** 단추를 클릭합니다.
3. **Azure Repos Git(YAML)** 선택
4. **eShopOnWeb** 리포지토리를 선택합니다.
5. **기존 Azure Pipelines YAML 파일**을 선택합니다.
6. **/.ado/eshoponweb-ci-pr.yml** 파일을 선택한 다음 **계속**을 클릭합니다.
...
8. 파이프라인은 프로젝트 이름을 기준으로 이름을 사용합니다. 파이프라인을 더 잘 식별하기 위해 **이름을 바꿔**보겠습니다. **파이프라인 > 파이프라인으로** 이동하여 최근에 만든 파이프라인을 클릭합니다. 줄임표 및 **이름 바꾸기/제거** 옵션을 클릭합니다. **이름을 eshoponweb-ci-pr으로** 지정하고 **저장**을 클릭합니다.
...
이 작업에서는 Azure DevOps 포털을 사용하여 새 분기를 사용하여 변경 사항을 보호된 **기본** 분기에 병합하는 끌어오기 요청을 만듭니다.

1. eShopOnWeb 탐색에서 **Repos** 섹션으로 이동하고 **분기**를 클릭합니다.
2. **기본** 분기에 따라 **Feature01**이라는 새 분기를 만듭니다.
3. **Feature01*을 클릭하고 **Feature01** 분기**의 일부로 /eShopOnWeb/src/Web/Program.cs** 파일로 이동하여 첫 번째 줄에서 다음을 변경합니다.

    ```csharp
    // Testing my PR
    ```

4. **커밋 > 커밋**을 클릭합니다(기본 커밋 메시지는 그대로 둠).
5. 메시지가 팝업되어 끌어오기 요청을 만들도록 제안합니다(**이제 Feature01** 분기가 **기본** 비해 변경 내용에 앞서 있음). **끌어오기 요청 만들기**를 클릭합니다.
6. **새 끌어오기 요청** 탭에서 기본값은 그대로 두고 **만들기**를 클릭합니다.
7. 끌어오기 요청은 대상 **기본** 분기에 적용된 정책에 따라 보류 중인 몇 가지 요구 사항을 표시합니다.
    - 1명 이상의 사용자가 변경 내용을 검토하고 승인해야 합니다.
    - 빌드 유효성 검사에서 빌드 **eshoponweb-ci-pr** 이 자동으로 트리거된 것을 볼 수 있습니다.

8. 모든 유효성 검사가 성공하면 오른쪽 위에서 **승인을** 클릭합니다. 이제 **자동 완성 설정** 드롭다운에서 **완료**를 클릭할 수 있습니다.
9. **끌어오기 요청 완료** 탭에서 **병합 완료**를 클릭합니다.
...
1. 이제 **트리거: none** 코드를 다음 코드로 바꿔야 합니다.
    ...
    기본 분기 및 웹 애플리케이션 코드(src/웹 폴더)를 변경하면 빌드 파이프라인이 자동으로 트리거됩니다.

    분기 정책을 사용하도록 설정했기 때문에 코드를 업데이트하려면 끌어오기 요청을 전달해야 합니다.

2. **저장** 단추(**저장 및 실행** 아님)를 클릭하여 파이프라인 정의를 저장합니다.
3. **이 커밋에 대한 새 분기 만들기를 선택합니다.**
4. 기본 분기 이름을 유지하고 **끌어오기 요청 시작을** 확인합니다.
5. 설정 메뉴에서 **저장**
6. 파이프라인은 프로젝트 이름을 기준으로 이름을 사용합니다. 파이프라인을 더 잘 식별하기 위해 **이름을 바꿔**보겠습니다. **파이프라인 > 파이프라인으로** 이동하여 최근에 만든 파이프라인을 클릭합니다. 줄임표 및 **이름 바꾸기/제거** 옵션을 클릭합니다. 이름을 **eshoponweb-ci**로 설정하고 **저장**을 클릭합니다.
7. **리포지토리 > 끌어오기 요청으로** 이동
8. 기존 끌어오기 요청을 클릭합니다.
9. 모든 유효성 검사가 성공하면 오른쪽 위에서 **승인을** 클릭합니다. 이제 **완료**를 클릭할 수 있습니다.
10. **끌어오기 요청 완료** 탭에서 **병합 완료**를 클릭합니다.
...
4. **/eShopOnWeb/src/Web/Program.cs** 파일로 이동하여 첫 번째 줄을 제거합니다.

    ```csharp
    // Testing my PR
    ```

5. **커밋 > 커밋**을 클릭합니다(기본 커밋 메시지는 그대로 둠).
6. 메시지가 팝업되어 끌어오기 요청을 만들도록 제안합니다(**이제 Feature02** 분기가 **기본** 비해 변경 내용이 앞당겨짐).
7. **끌어오기 요청 만들기**를 클릭합니다.
8. **새 끌어오기 요청** 탭에서 기본값은 그대로 두고 **만들기**를 클릭합니다.
9. 끌어오기 요청에는 대상 **기본** 분기에 적용된 정책에 따라 보류 중인 몇 가지 요구 사항이 표시됩니다.
10. 모든 유효성 검사가 성공하면 오른쪽 위에서 **승인을** 클릭합니다. 이제 **자동 완성 설정** 드롭다운에서 **완료**를 클릭할 수 있습니다.
11. **끌어오기 요청 완료** 탭에서 **병합 완료**를 클릭합니다.
12. **Pipelines>Pipelines로 돌아가기** 코드가 병합된 후 빌드 **eshoponweb-ci**가 자동으로 트리거된 것을 알 수 있습니다.
13. **eshoponweb-ci** 빌드를 클릭한 다음 마지막 실행을 선택합니다.
14. 성공적으로 실행된 후 **관련 > 게시됨을** 클릭하여 게시된 아티팩트 검사.
    - Bicep: 인프라 아티팩트입니다.
    - 웹 사이트: 앱 아티팩트입니다.
