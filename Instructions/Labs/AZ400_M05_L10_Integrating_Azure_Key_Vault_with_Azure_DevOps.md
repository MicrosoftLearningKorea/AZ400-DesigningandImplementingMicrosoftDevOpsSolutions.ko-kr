...
3. **키 자격 증명 모음 만들기** 블레이드의 **기본** 탭에서 다음 설정을 지정하고 **다음**을 클릭합니다.
...
4. **키 자격 증명 모음** 블레이드의 **액세스 정책** 탭에 있는 **액세스 정책** 섹션에서 **+ 만들기**를 클릭하여 새 정책을 설정합니다.
    ...
    2. **보안 주체** 블레이드에서 지정된 ID 또는 이름을 사용하여 **이전에 만든 서비스 주체**를 검색합니다. **다음**을 클릭하고 **다음**을 다시 클릭합니다.
    3. **검토 + 만들기** 블레이드에서 **만들기**를 클릭합니다.

5. **키 자격 증명 모음 만들기** 블레이드로 돌아가서 **검토 + 만들기 > 만들기**를 클릭합니다.

    > **참고**: Azure Key Vault가 프로비전될 때까지 기다립니다. 이는 1분 이내에 완료됩니다.

6. **배포가 완료됨** 블레이드에서 **리소스로 이동**을 클릭합니다.
7. Azure Key Vault 블레이드 왼쪽의 세로 메뉴에 있는 **개체** 섹션에서 **비밀**을 클릭합니다.
