...
1. 랩 컴퓨터에서 웹 브라우저를 시작하고[**, Azure Portal**](https://portal.azure.com)로 이동하고, 이 랩에서 사용할 Azure 구독의 소유자 역할이 있고 이 구독과 연결된 Microsoft Entra 테넌트에서 전역 관리자 역할을 가진 사용자 계정으로 로그인합니다.
...
4. 키 자격 **증명 모음** **만들기** 블레이드의 액세스 구성 탭에서 **자격 증명 모음 액세스 정책을** 선택한 다음 **액세스 정책** 섹션에서 **+ 만들기** 를 클릭하여 새 정책을 설정합니다.
    ...
    1. **권한** 블레이드의 **비밀 권한** 아래에서 **권한 가져오기** 및 **나열**을 검사. **다음**을 클릭합니다.
...
5. **저장 및 실행을** 클릭합니다.
6. 파이프라인을 열고 성공적으로 실행될 때까지 기다립니다.

    > **중요**: "이 파이프라인은 이 실행이 ACI에 대한 Docker Compose를 계속 진행하기 전에 리소스에 액세스할 수 있는 권한이 필요합니다"라는 메시지가 표시되면 보기, 허용 및 허용을 다시 클릭합니다. 파이프라인이 리소스를 만들 수 있도록 하려면 이 작업이 필요합니다.
