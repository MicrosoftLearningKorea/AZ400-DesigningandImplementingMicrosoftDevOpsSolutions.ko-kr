...
1. EShopOnWeb** 프로젝트가 열려 있는 **Azure DevOps 포털을 표시하는 웹 브라우저에서 왼쪽의 세로 탐색 창에서 파이프라인**을 클릭합니다**.
...
6. **기존 YAML 파일** 선택에서 분기에 **대한 기본** 선택하고 경로에 대해 /.ado/eshoponweb-ci-pr.yml**을 선택합니다**.
7. **계속**을 클릭합니다.
8. **파이프라인 YAML** 검토 창에서 샘플 파이프라인을 검토합니다. 이는 다음을 수행하는 다소 직선적인 .NET 애플리케이션 빌드 파이프라인입니다.
- 단일 스테이지: 빌드
- 단일 작업: 빌드
- 빌드 작업 내의 3개 작업:
- Dotnet Restore
...
9. 파이프라인 YAML 검토 창에서 **실행** 단추 옆에 있는 아래쪽으로 향한 캐리트 기호를 **클릭하고 저장**을 클릭합니다**.**
...
1. Lab VM(Lab Virtual Machine) 또는 개인 컴퓨터를 내에서 웹 브라우저를 시작하고 [Azure DevOps 포털](https://dev.azure.com)로 이동한 다음 Azure DevOps 조직과 연결된 Microsoft 계정을 사용하여 로그인합니다.
  ...
  > **참고**: 랩 가상 머신에는 필요한 모든 필수 구성 요소 소프트웨어가 설치되어 있어야 합니다. 사용자 고유의 컴퓨터에 설치하는 경우 데모 프로젝트를 빌드하는 데 필요한 .NET 6.0.x SDK 이상을 설치해야 합니다. .NET[ 다운로드를 참조하세요](https://dotnet.microsoft.com/download/dotnet).
...
8. 에이전트 풀** 추가 창**의 **풀 유형** 드롭다운 목록에서 자체 호스팅****을 선택하고 **이름** 텍스트 상자에 az400m03l03a-pool**을 입력**한 다음 만들기**를 클릭합니다**.
9.  **에이전트 풀** 창으로 돌아가서 새로 만든 **az400m03l03a-pool**을 나타내는 항목을 클릭합니다.
...
11. **에이전트 가져오기** 창에서 **Windows** 및 **x64** 탭이 선택되어 있는지 확인하고 **다운로드**를 클릭하여 에이전트 이진 파일이 포함된 zip 보관 파일을 사용자 프로필 내의 로컬 **다운로드** 폴더에 다운로드합니다.
    ...
    > **참고**: 현재 시스템 설정으로 인해 파일을 다운로드할 수 없음을 나타내는 오류 메시지가 표시되면 브라우저 창의 오른쪽 위 모서리에서 설정** 메뉴 머리글을 지정하는 **기어 휠 기호를 클릭하고 드롭다운 메뉴에서 인터넷 옵션을 선택하고 **인터넷 옵션**** 대화 상자에서 **고급을 클릭하고 고급 탭에서 **고급**** 을 클릭합니다**.  다시 설정을** 클릭하고 **브라우저 설정** 다시 설정 대화 상자에서 **다시 설정을** 클릭하고 **닫기를** 클릭한 **다음 다운로드를 다시 시도합니다.
...
20. EShopOnWeb** 창의 왼쪽**에 있는 세로 탐색 창의 파이프라인 섹션에서 파이프라인****을 클릭합니다**.**
...
23. **EShopOnWeb** 편집 창의 창 오른쪽 위 모서리에서 저장**을 클릭하고 **저장** 창에서 **다시 저장**을 클릭합니다**. 그러면 이 파이프라인을 기반으로 하는 빌드가 자동으로 트리거됩니다.
24. Azure DevOps 포털 왼쪽의 세로 탐색 창에 있는 **Pipelines** 섹션에서 **파이프라인**을 클릭합니다.
25. **파이프라인** 창의 **최근** 탭에서 EShopOnWeb 항목을 클릭하고 **EShopOnWeb**** 창의 **실행** 탭에서 **가장 최근 실행을 **** 선택하고 실행 요약 창에서 아래쪽**으로 스크롤한 다음 작업** 섹션에서 1**단계를 클릭하고 **작업이 성공적으로 완료될 때까지 작업을 모니터링합니다.

### 연습 3: 랩에서 사용되는 리소스 제거

1. 명령 프롬프트에서 실행 `.\config.cmd remove` 하여 에이전트 서비스를 중지하고 제거합니다.
2. 에이전트 풀을 삭제합니다.
3. PAT 토큰을 해지합니다.
