...
1. **파이프라인>파일로** 이동합니다.
2. **파이프라인 만들기 단추(파이프라인**이 없는 경우) 또는 **새 파이프라인 단추(이미 생성된 파이프라인**이 있는 경우)를 클릭합니다.
...
2. **앱 구성 만들기**를 클릭한 후 다음을 선택합니다.
    - Azure 구독.
    - 이전에 만든 리소스 그룹입니다(AZ400-EWebShop-NAME**이라는 이름을 **지정해야 합니다).
    - 위치입니다.
    - 예를 들어 appcs-NAME-REGION**과 같은 **고유한 이름입니다.
    - 무료** 가격 책정 계층을 **선택합니다.
...
2. **설정** 섹션에서 ID**를 클릭한 **다음 시스템 할당** 섹션에서 상태 켜**** 짐으로 **전환하고 > 저장**을 클릭하고 **작업이 완료되기까지 몇 초 정도 기다립니다.
...
3. 새 애플리케이션 설정 두 가지를 추가합니다.
    ...
    - 두 번째 앱 설정
        ...
        - **값:** *이전에 App Configuration 엔드포인트에서 저장/복사한 값. 다음과 같이 표시됨https://appcs-NAME-REGION.azconfig.io*

4. **확인**을 클릭한 다음, **저장**을 클릭하고 설정이 업데이트될 때까지 기다립니다.
